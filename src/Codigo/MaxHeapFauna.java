 ...
 * @author danie
 */
public class MaxHeapFauna {
    private Fauna[] Heap;
    private int size;
    private int maxsize;
    private static final int FRONT = 1;

    public MaxHeapFauna(int size){
        this.maxsize = size;
        this.size = 0;
        Heap = new Fauna[this.maxsize + 1];
        Heap[0] = new Fauna(Integer.MAX_VALUE);
        for(int i=0;i<size;i++){
            Heap[i+1]=(new Fauna(0));
        }
    }
    ...
    public void insert(Fauna element) {
        ...
        Heap[size].alimentacion=element.alimentacion;
        Heap[size].depredador=element.depredador;
        int current = size;
